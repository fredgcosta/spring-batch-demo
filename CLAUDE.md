...
- `docker/docker-compose*.yml` spin up the full SCDF environment (Data Flow server, Skipper, Kafka, Postgres, pgAdmin) plus a single `grafana/otel-lgtm` all-in-one observability container (bundling OTel Collector + Prometheus + Tempo + Loki + Grafana) — see README for the exact `docker-compose` invocation with env vars. The `dataflow-server` service builds a local image from `docker/Dockerfile` (base image `springcloud/spring-cloud-dataflow-server:2.11.5-jdk17`). `docker/docker-compose-prometheus.yml` adds the `lgtm` service (`grafana/otel-lgtm`) that receives OTLP on 4317/4318 and serves Grafana (3000), the embedded Prometheus UI (9090) and Tempo (3200); the old `prometheus-rsocket-proxy` and the separate otel-collector/prometheus/grafana containers (and `docker/otel-collector-config.yaml`) were removed.
- Grafana dashboard JSON files live in `grafana/` for the SCDF observability setup.
